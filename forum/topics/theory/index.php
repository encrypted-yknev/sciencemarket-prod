...
if($_SERVER['REQUEST_METHOD'] == 'POST')	{
	...
	if(!empty($topic_nm) && !empty($sort_order))	{
		...
		if($sort_order == 'My posts')	{
			$sql.="b.parent_topic = 7 and a.posted_by='".$_SESSION['user']."' order by ".$sort." desc";
		}
		else	{
			$sql.="b.parent_topic = 7 order by ".$sort." desc";
		}
	}
	else 	{
		echo "empty";
		$sql.="b.parent_topic = 7 order by created_ts desc";
	}
}
else	{
	$sql="select a.qstn_id,a.qstn_titl,a.qstn_desc,a.topic_id,a.posted_by,a.up_votes,a.down_votes,a.created_ts from questions a inner join 	topics b on a.topic_id = b.topic_id where b.parent_topic = 7 order by created_ts desc";
}
			...
			<div class="col-sm-10">
				<form class="form-inline" id="filter-form" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
					<div class="form-group" id="filter-section">
						...
						<?php
							try	{
								$sql_fetch_sub_topics = "select topic_id,topic_desc from topics where parent_topic = 7";
